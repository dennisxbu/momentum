    ...
    <div className="app-shell">
      <aside className="sidebar">
        ...
        <div className="sidebar-foot">
          ...
          <div><strong>Lokal geschützt</strong><small>Keine Momentum-Cloud · kein Konto</small></div>
        </div>
      </aside>
...
function DataControl({ workspace, busy, onExport, onRestore, onReset }: { workspace: WorkspaceView; busy: boolean; onExport: () => void; onRestore: () => void; onReset: () => void }) {
  return <div className="page data-page"><header className="page-heading"><div><span className="overline">Datenhoheit</span><h1>Deine Grundlage bleibt bei dir.</h1><p>Der Momentum-Kern verwendet in diesem Stand kein Konto und keinen Onlinedienst. Deine gespeicherten Inhalte liegen in der lokalen, verschlüsselten Datenbank.</p></div></header><section className="data-summary"><div className="shield">✓</div><div><h2>Lokale Datenbank verschlüsselt</h2><p>{workspace.storage_label}</p><small>Der Schlüssel ist an dein Windows-Benutzerkonto gebunden. Die Windows-WebView2-Laufzeit kann unabhängig von Momentum technische Herstellerverbindungen aufbauen.</small></div></section><section className="data-actions"><article><span>Mitnehmen</span><h2>Vollständigen Export speichern</h2><p>Ein lesbarer JSON-Export enthält Struktur, Verlauf und aktuellen Stand. Wähle einen sicheren Ablageort.</p><button className="action primary" disabled={busy} onClick={onExport}>Exportieren</button></article><article><span>Zurückholen</span><h2>Geprüft wiederherstellen</h2><p>Momentum prüft den gesamten Export zuerst. Erst ein gültiger Bestand ersetzt die lokalen Daten.</p><button className="action secondary" disabled={busy} onClick={onRestore}>Export auswählen</button></article><article><span>Entwicklungsstand</span><h2>Synthetischen Fall neu beginnen</h2><p>Setzt ausschließlich die künstlichen Prüfdaten zurück. In diesem Entwicklungsstand sind keine echten persönlichen Daten vorgesehen.</p><button className="action quiet danger" disabled={busy} onClick={onReset}>Prüffall zurücksetzen</button></article></section></div>;
}
